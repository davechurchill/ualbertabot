...
void Player_KiterDPS::getMove(const GameState & state, Move & move)
{
    MoveArray moves;
	...
	for (size_t u(0); u<moves.numUnits(); ++u)
	{
		...
		const Unit & closestUnit				(ourUnit.canHeal() ? 
                                                AITools::GetClosestOurUnit(state, _playerID, u) : AITools::GetClosestEnemyUnit(state, _playerID, u));
...
PlayerPtr Player_KiterDPS::clone()
{
    return PlayerPtr(new Player_KiterDPS(*this));
}