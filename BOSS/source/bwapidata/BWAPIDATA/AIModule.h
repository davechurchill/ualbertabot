...
namespace BWAPIDATA
{
   ...
   *
   * \note
   * Using BWAPIDATA in a different thread than the default one will produce unexpected results and possibly crash
   * the program. Multi-threaded AIs are possible so long as all BWAPIDATA interaction is limited to the default
  ...
  class AIModule
  {
    public:
      ...
      /** BWAPIDATA calls this at the start of a match. Typically an AI will execute set up code in this method
       * (initialize data structures, load build orders, etc). */
      virtual void onStart();

      /** BWAPIDATA calls this at the end of the match. isWinner will be true if the AIModule won the game. If the
       * game is a replay, isWinner will always be false. */
      virtual void onEnd(bool isWinner);

      /** BWAPIDATA calls this on every logical frame in the game. */
      virtual void onFrame();

      /** If Flag::UserInput is enabled, BWAPIDATA will call this each time a user enters a message into the chat.
       * */
      virtual void onSendText(std::string text);

      /** BWAPIDATA calls this when another player sends a message. */
      virtual void onReceiveText(Player* player, std::string text);

      /** BWAPIDATA calls this when a player leaves the game. */
      virtual void onPlayerLeft(Player* player);

      /** BWAPIDATA calls this when a nuclear launch has been detected. If the target position is visible, or if
      ...
      /** BWAPIDATA calls this when a unit becomes accessible. */
      virtual void onUnitDiscover(Unit* unit);

      /** BWAPIDATA calls this when a unit becomes inaccessible. */
      virtual void onUnitEvade(Unit* unit);

      /** BWAPIDATA calls this the instant a previously invisible unit becomes visible. The complete map
       * information flag has no effect on this callback. */
      virtual void onUnitShow(Unit* unit);

      /** BWAPIDATA calls this right before a unit becomes invisible, so if you want your non-cheating AI to
      ...
      /** BWAPIDATA calls this when a unit is created. Note that this is NOT called when a unit changes type
      ...
      /** BWAPIDATA calls this when a unit dies or otherwise removed from the game (i.e. a mined out mineral
      ...
      /** BWAPIDATA calls this when a unit changes type, such as from a Zerg Drone to a Zerg Hatchery, or from a
      ...
      /** BWAPIDATA calls this when an accessible unit changes ownership. */
