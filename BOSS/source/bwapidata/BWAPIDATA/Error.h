...
namespace BWAPIDATA
{
  ...
  /** Functions in BWAPIDATA may set an error code. To retrieve the error code, call Game::getLastError. */
  class Error
  {
    public:
      Error();
  ...
  namespace Errors
  {
    ...
    /** Returned when attempting to order a unit that BWAPIDATA does not own (i.e. can't order enemy army to go
     *  away) */
