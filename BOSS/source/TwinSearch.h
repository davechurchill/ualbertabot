...
#include "StarcraftSearch.h"
#include "SearchConstraint.h"
...
{
	SearchParameters<GameStateType> params;		// parameters that will be used in this search
	
	int upperBound; 									// the current upper bound for search
			
	GameStateType winner;							// the winning state so far
...
public:
	...
	TwinSearch<GameStateType>(SearchParameters<GameStateType> p) 
		: params(p)
		, nodesExpanded(0)
		, numChildren(0)
		, numGenerations(0)
	...
	void DFBBMonteCarlo(GameStateType & s, int depth)
	{		
	...
	// recursive function which does all search logic
	void DFBB(GameStateType & s, int depth)
	{		
		// increase the node expansion count
		nodesExpanded++;
